...
int main(int argc, char *argv[]) {
    int                sd, nread, nwrite, port, nome_len;
    char               ok, buff[DIM_BUFF], nome_dir[LENGTH_FILE_NAME];
    struct hostent    *host;
    ...
    while (gets(nome_dir)) {
        ...
        nome_len = strlen(nome_dir);
        if (write(sd, &nome_len, sizeof(int)) < 0) {
            perror("write");
            close(sd);
            printf("Nome del file da richiedere: ");
            continue;
        ...
        if (ok == 'S') {
            printf("Directory trovata:\n");
			while((nread=read(sd, &nome_len, sizeof(int))) > 0) {
				read(sd, &buff, DIM_BUFF);

				if(write(1, buff, DIM_BUFF) < 0) {
					perror("write");
					break;
				}
			}
			printf("Client: fine trasmissione\n");
